...
| Date Updated | 2025-10-31 - Vex - Updated for Result<T> returns, centralized error handling, and exit codes                                                     |
|              | 2025-11-01 - Vex - CliUtil global using alias Cli. CliUtil redesign to handle user interface and machine-readable data output separately         |
|              | 2025-11-03 - Vex - Added dual Execute methods documentation and clarified CmdBase helper methods usage                                           |
|              | 2025-11-11 - Vex - Added StartCmd default execution when no arguments provided                                                                   |
|              | 2025-11-11 - Vex - Added Overview/Features sections; documented features: Start page and ICommandExecutor usage                                  |
|              | 2025-11-26 - Vex - Added Lifecycle Hooks section; documented features: Lifecycle Hooks, CommandContext, and Execution Order                      |
|              | 2025-12-31 - Vex - Added Formatting section with mixed colors XML tags and global margins; updated quick start examples to use CliBase wrappers; |
|              | 2026-01-30 - Vex - Added CliService for consistent CLI output across all components; replaced CliBase with injectable CliService architecture  |
...
│   ├── CliUtil.cs                       # Static CLI helpers (deprecated in favor of CliService)
...
│   ├── ParsedArguments.cs               # Parsed CLI arguments
│   └── CommandNode.cs                   # Command hierarchy nodes
│
├── CliService.cs                   # Injectable CLI output service
...
└── CliEngineExtensions.cs          # DI setup extensions
```
...
Vexit.CliEngine provides `CliService` for consistent, injectable CLI output across all components. Unlike static utilities, `CliService` can be configured centrally and injected anywhere in your application.

### Setup

Configure CliService in your `Program.cs`:
...
Inject and use CliService in any component:
...
public class MyService
{
    ...
    public MyService(CliService cli)
    {
        this.cli = cli;
    }
    ...
    public void DoWork()
    {
        cli.WriteLn("Starting work...");
        cli.WriteLnSuccess("Work completed!");
    }
}
```
...
- **Consistent Styling**: Same colors and formatting everywhere
...
public class GreetCmd : CmdBase
{
    ...
    public override Result Execute()
    {
        ...
        {
            return Result.Failure("Name is required", "MISSING_ARGUMENT");
        }
        ...
        for (int i = 0; i < Times; i++)
        {
            if (UseColours)
            {
                if (World)
                {
                    WriteLnFormat($"<s>Hello, {Name}!</s> <d>...and </d> <w>hello World</w><d>!</d>");
                }
                else
                {
                    WriteLnFormat($"<i>Hello, {Name}!</i>");
                }
            }
            else
            {
...
- **CliService Injection:** Using `CliService` for consistent CLI output. Inject `CliService` into your classes and use methods like `WriteLn()` and `WriteLnFormat()` for automatic global indentation. You can also use `CliUtil` methods directly if needed
- **Error Handling:** `Result.Failure()` for validation errors
...
**Available Tags:**
- `<i>text</i>` - Info color (cyan)
- `<s>text</s>` - Success color (green)
- `<w>text</w>` - Warning color (yellow)
- `<e>text</e>` - Error color (red)
- `<d>text</d>` - Dim color (gray)
- `<l>text</l>` - Lite color (white)

**Example Usage:**

```csharp
// Mixed colors in one line
WriteLnFormat($"<s>Success:</s> <i>Info message</i> with <w>warning</w> and <e>error</e> highlights");

// Single context colors (alternative)
WriteLnSuccess("This entire line is green");
WriteLnWarning("This entire line is yellow");
```
...
Always use the `CliService` methods (`WriteLn`, `WriteLnFormat`, `WriteLnSuccess`, etc.) instead of calling `CliUtil` directly. These methods automatically apply the global margins.
...
public class ExampleCmd : CmdBase
{
    ...
    public ExampleCmd(CliService cli)
    {
        this.cli = cli;
    }
    ...
    public override Result Execute()
    {
        ...
        cli.WriteLn("This text will be indented by VEXIT_CLI_LEFT_MARGIN spaces");
        cli.WriteLnFormat("<s>Success message</s> with <w>warning highlight</w>");
        cli.WriteLnSuccess("This success message is automatically indented");

        // DON'T do this - bypasses global margins:
        // CliUtil.WriteLn("This won't respect margins");
...
To achieve programmatic command execution, inject the `ICommandExecutor` service and call the `Execute` method of the service like this:
  ...
  ```csharp
  public class MyInteractiveCmd : CmdBase
  {
      private readonly ICommandExecutor _executor;

      public MyInteractiveCmd(ICommandExecutor executor)
      {
          _executor = executor;
      }

      public override async Task<Result> ExecuteAsync(string[] args, Dictionary<string, string> options)
      {
          // Single command execution
          var userInput = Cli.ReadLn(); // e.g., "init --force"
...
- **Interactive Shell Example:** Use ICommandExecutor inside Start Page
  ...
  public class StartCmd : CmdBase
  {
      ...
      public override async Task<Result> ExecuteAsync(string[] args, Dictionary<string, string> options)
      {
          ...
          while (true)
          {
              Cli.Write("Enter command or hit Enter to exit: ");
              var input = Cli.ReadLnDim();

              if (string.IsNullOrWhiteSpace(input))
                  return Result.Ok(); // Exit shell

              // Execute command and continue loop
              var result = await _executor.Execute(input.Trim());
              if (result.IsFailure) Cli.WriteLn(); // Add spacing
          }
      }
  }
  ```
...
public class RunCmd : CmdBase
{
    [Option("file", "f", "Path to file containing commands to execute", required: true)]
    public string? FilePath { get; set; }

    private readonly ICommandExecutor _commandExecutor;

    public RunCmd(ICommandExecutor commandExecutor)
    {
    ...
    public override async Task<Result> ExecuteAsync()
    {
        if (string.IsNullOrWhiteSpace(FilePath) || !File.Exists(FilePath))
            return Result.Failure("Valid file path required");
        ...
        foreach (var line in lines)
        {
            ...
            {
                Cli.WriteLnWarning($"Skipping non-yourcli command: {trimmedLine}");
                continue;
            }
            ...
            try
            {
                ...
                {
                    failedCount++;
                    Cli.WriteLnError($"Command failed: {result.Message}");

                    // Smart continue: only ask if more commands exist
                    var remainingCommands = validCommands - processedCount;
                    if (remainingCommands > 0)
                    {
                        var continueResult = Cli.PromptYesNo($"Continue with remaining {remainingCommands} commands?", true);
                        if (!continueResult)
                        {
                            Cli.WriteLnDim("Execution stopped by user.");
                            break;
                        }
                    }
                }
                ...
                {
                    executedCount++;
                    Cli.WriteLnSuccess("Command completed successfully");
                }
            }
            catch (Exception ex)
            {
                failedCount++;
                Cli.WriteLnError($"Command execution failed: {ex.Message}");
        ...
        Cli.WriteLnDim($"Execution summary: {executedCount} successful, {failedCount} failed");
        return failedCount > 0 ? Result.Failure("Some commands failed") : Result.Success();
    }
}
```
...
- **Registration:**
  ...
  builder.Services.AddHook<TempFolderCleanup_OnBeforeExecute>();
  builder.Services.AddHook<CheckUpdates_OnStartup>();
  builder.AddCliEngine();
  ```
  
  > `AddHook<T>()` extension method is provided by CliEngine and registers your hook implementation with dependency injection. Hooks are resolved from DI and executed automatically by the framework.
...
- **AddCliEngine**: Extension on IHostApplicationBuilder to add the framework with options for assembly scanning.
...
- **DI Integration**: Uses Microsoft.Extensions.DependencyInjection for optional service registration and dependency injection.
- **Argument Parsing**: Supports **positional** - Arguments, **named** - Options (long/short), and key=value formats with validation.
- **Convention-Based Command Discovery**: Commands are organized by folder structure, and the library automatically infers the command hierarchy from your namespace.
...
- **Separate Output Streams** - for Human-Facing Output (stderr) and Machine-Readable Data (stdout) via use of CliUtil as alias Cli.
...
public class ConsoleLogger : ILogger { public void Log(string msg) => Cli.WriteLn(msg); }

[Command("hello", "Say hello with DI")]
public class HelloCmd : CmdBase
{
    private readonly ILogger _logger;
    public HelloCmd(ILogger logger) { _logger = logger; }
...
if (result.IsFailure)
{
    // Optional: Print error centrally (commands may have already printed and not passed it, in which case HasMessage will be false)
    if (result.HasMessage)
    {
        CliUtil.WriteError(result.Message!);
    }

    // Explicit exit code for automation (e.g., CI/CD)
    Environment.Exit(1);
}
...
### 1. Result and Command Execution
Commands return structured `Result` objects for two key reasons:

#### 1. **Explicit Exit Codes for Automation**
...
#### 2. **Centralized Error Handling (Optional)**
...
- **Success**: `return Ok();`
- **Logic Failure**: `return Failure("Error message", "ERROR_CODE");`
- **System Error**: `return Failure(ex);` - auto-sets `IsError`
- **Proc Result Chaining**: `return Failure(procResult);` - converts `Result<T>` to `Result`
- **Centralized Handling**: In `Program.cs`, check `result.HasMessage` to print errors centrally.


### 2. Output Management with CliUtil
CliEngine provides `CliUtil` for consistent, stream-aware output. **Never use `Console.WriteLine` directly** - always use CliUtil methods to ensure proper stream separation.
...
This makes `Cli` available everywhere in your project without local `using` statements.

#### **Human-Facing Output (stderr)**
- Use `Cli.WriteLn()`, `Cli.WriteError()`, `Cli.WriteWarning()`, `Cli.WriteSuccess()`, etc.
- All output goes to `Console.Error` (stderr) to keep stdout clean for data.
- Supports colors and formatting for better UX.
- Example: `Cli.WriteLn("Deploying...", ConsoleColor.Cyan);`

#### **Machine-Readable Data (stdout)**
- Use `Cli.WriteData<T>(data, format)` for structured output.
- Writes to `Console.Out` (stdout) for easy piping in scripts/CI.
- Supports `DataFormat.Json` (default) and `DataFormat.Text` (ToString).
- Example: `Cli.WriteData(deploymentResult, DataFormat.Json)` outputs indented JSON.
...
public class DeployCmd : CmdBase
{
    public override async Task<Result> ExecuteAsync()
    {
        Cli.WriteLn("🚀 Starting deployment...", ConsoleColor.Cyan);

        // Simulate deployment work
        await Task.Delay(1000);

        Cli.WriteLn("✅ Deployment complete!", ConsoleColor.Green);

        // Output structured result data to stdout
        var result = new { Status = "success", Version = "1.0.0" };
        Cli.WriteData(result, DataFormat.Json);
...
### 4. Dual Execute Methods: Synchronous vs Asynchronous Commands
...
public class SyncCmd : CmdBase
{
    public override Result Execute()
    {
        Cli.WriteLn("Processing synchronously...");
...
public class AsyncCmd : CmdBase
{
    public override async Task<Result> ExecuteAsync()
    {
        Cli.WriteLn("Starting async operation...");
        await Task.Delay(1000); // Async work
        Cli.WriteLn("Async operation complete!");
        return Ok();
    }
}
```
...
public class GreetCmd : CmdBase
{
    [Option("name", "n", "Name(s) to greet", IsMultiValue = true)]
    ...
    public override Result Execute()
    {
        // Build the full name from multiple names
        string fullName = string.Join(" ", Names);

        // Ensure we have something to greet
        if (string.IsNullOrWhiteSpace(fullName) && !World)
        {
            return Failure("Either provide name(s) with --name or use --world flag", "MISSING_GREETING_TARGET");
        }

        // Greet the person (if name provided)
        if (!string.IsNullOrWhiteSpace(fullName))
        {
            for (int i = 0; i < Times; i++)
            {
                Cli.WriteLn($"Hello, {fullName}!");
            }
        }

        // Greet the world (if flag set)
        if (World)
        {
            for (int i = 0; i < Times; i++)
            {
...
- **Multi-value options**: `Names` list accumulates multiple `--name` values
...
- **CliUtil usage**: Uses `Cli.WriteLn()` for consistent output
...
public class ArgumentAttribute : Attribute
{
    ...
    public ArgumentAttribute(string name, string description = "", bool isRequired = true)
}
```
...
### MultiValue Options
```csharp
[Option("tag", "t", "Add a tag", IsMultiValue = true)]
...
public class InitCmd : SecretsCmd  // Inherits ServerPath option
{
    ...
    public override Result Execute()
    {
        Cli.WriteLn($"🔧 Initializing secrets at: {ServerPath}/{SecretsDir}");

        // Your implementation here

        Cli.WriteLn("✅ Secrets initialized!");
        return Ok();
    }
}
```
...
public class InitCmd : SecretsCmd  // Inherits ServerPath option
{
    ...
    public override Result Execute()
    {
        ...
        Cli.WriteLn($"Initializing at: {ServerPath}/{SecretsDir}");
        return Ok();
    }
}
```
...
public class StartCmd : CmdBase
{
    ...
    {
        Cli.WriteLn("╔══════════════════════════════════════════════╗");
        Cli.WriteLn("║            Welcome to YourApp!              ║");
        Cli.WriteLn("╚══════════════════════════════════════════════╝");
        Cli.WriteLn();
        Cli.WriteLn("Quick Actions:");
        Cli.WriteLn("  yourapp init     - Initialize new project");
        Cli.WriteLn("  yourapp build    - Build application");
        Cli.WriteLn("  yourapp deploy   - Deploy to production");
        Cli.WriteLn();
        Cli.WriteLn("Use 'yourapp --help' for detailed help");
...
### Documentation Tasks
- [x] Document hook interface design and methods
- [x] Document CommandContext structure
- [x] Document registration mechanism
- [x] Document execution order and behavior
- [x] Document exception handling strategy
- [x] Document use cases and examples
- [ ] Add hook examples to command examples section (if needed)

### CliEngine Framework Implementation Tasks
- [ ] Create `ICommandLifecycleHook` interface in `Vexit.CliEngine/Interfaces/`
- [ ] Create `CommandContext` class in `Vexit.CliEngine/Models/`
- [ ] Add `AddCommandHook<T>()` extension method to `CliEngineExtensions.cs`
- [ ] Update `CommandController.ExecuteCommand()` to:
  - [ ] Resolve hooks from DI (`IEnumerable<ICommandLifecycleHook>`)
  - [ ] Sort hooks by `Order` property
  - [ ] Execute blocking hooks before command (await, check results)
  - [ ] Execute non-blocking hooks before command (fire-and-forget)
  - [ ] Execute after hooks after command (fire-and-forget)
  - [ ] Add exception handling for all hook executions
- [ ] Add unit tests for hook execution order
- [ ] Add unit tests for hook failure scenarios
- [ ] Add unit tests for non-blocking hook behavior

### VX CLI Consumer Implementation Tasks
- [ ] Create `TempFolderCleanupHook` class in `Vexit.VxCli/Hooks/`
- [ ] Implement cleanup logic (scan all `~/.vexit/projects/*/_temp` folders)
- [ ] Implement warning output for failed deletions
- [ ] Register hook in `Program.cs` via `AddCommandHook<TempFolderCleanupHook>()`
- [ ] Test hook execution with various commands
- [ ] Test hook behavior when `_temp` folders are locked
