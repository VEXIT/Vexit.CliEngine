 ...
 *                  2025-11-30	| Vex | Expanded scanning to include the root Services namespace so commands can combine ServiceGroups with slice-level services
 ************************************************/
...
/// within a specific namespace subtree. Enables true plug-and-play modularity for commands.
/// </summary>
public class ConventionBasedServiceRegistry : IServiceRegistry
{
    ...
    /// <summary>
    /// Registers all discovered services within the namespace subtree. <br />
    /// Services are registered as transient by default.
    /// </summary>
    /// <param name="services">The service collection to register services with.</param>
    public void RegisterServices(IServiceCollection services)
    {
        var servicesNamespaceRoot = $"{_namespacePrefix}.Services";
        // Console.WriteLine($"[DEBUG ConventionRegistry] Scanning namespace: {servicesNamespaceRoot}");
        ...
        var serviceTypes = _assembly.GetTypes()
            .Where(t => t.Namespace != null &&
                       ...
                       t.GetCustomAttribute<CommandAttribute>() == null) // Exclude commands themselves
            .ToList();

        // Console.WriteLine($"[DEBUG ConventionRegistry] Found {serviceTypes.Count} service types");

        foreach (var serviceType in serviceTypes)
        {
            // Console.WriteLine($"[DEBUG ConventionRegistry] Registering: {serviceType.FullName}");

            var interfaces = serviceType.GetInterfaces()
                .Where(i => i != typeof(IServiceRegistry))
                .ToList();

            // Register the concrete type if not already present to avoid overriding explicit registrations.
            services.TryAddTransient(serviceType);

            // If there are interfaces, register them to point to the concrete type's instance.
            // This ensures that both IMyService and MyService resolve to the same object.
            if (interfaces.Any())
            {
                foreach (var @interface in interfaces)
                {
                    // Only add if not already registered; preserves precedence of ServiceGroups and Modules.
